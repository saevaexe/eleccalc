# ElecCalc - Electrical Engineering Calculator

## Build & Test
```bash
xcodebuild -scheme MyApp -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17 Pro' build
xcodebuild test -scheme MyApp -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17 Pro'
```
...
- Premium gate: `category.isPremium && !subscriptionManager.isSubscribed` — never bypass this check
- History: save via `CalculationRecord` SwiftData model after every successful calculation
- Subscription: StoreKit 2 native (not RevenueCat) — SubscriptionManager.shared singleton
...
4. `MyAppTests/XxxEngineTests.swift` — min 5 test cases
5. Add case to `CalculationCategory` enum + set isPremium
6. Add navigation in `ContentView`
...
- Add third-party dependencies without explicit approval — zero external deps, StoreKit 2 only
- Use ObservableObject or @StateObject — this project uses iOS 17+ @Observable only
...
- Test target imports `@testable import ElecCalc` (not MyApp)
- pbxproj edits: always verify ref IDs don't collide with existing ones
- Onboarding state: `UserDefaults.standard.bool(forKey: OnboardingViewModel.hasCompletedKey)`
