...
SCREENSHOTS = {
    "en": {
        "home": os.path.join(OUT_DIR, "home_en.png"),
        "paywall": os.path.join(OUT_DIR, "paywall_en.png"),
        "home_ipad": os.path.join(OUT_DIR, "home_ipad_en.png"),
        "paywall_ipad": os.path.join(OUT_DIR, "paywall_ipad_en.png"),
    },
    "tr": {
        "home": os.path.join(OUT_DIR, "home_tr.png"),
        "paywall": os.path.join(OUT_DIR, "paywall_tr.png"),
        "home_ipad": os.path.join(OUT_DIR, "home_ipad_tr.png"),
        "paywall_ipad": os.path.join(OUT_DIR, "paywall_ipad_tr.png"),
    },
}
...
TEXTS = {
    "en": {
        ...
        "hero_2": "Electrical Calculators",
        "hero_sub_1": "Engineering calculations",
        "hero_sub_2": "now in your pocket",
        "feat_title_1": "All Calculations",
        ...
        "features": [
            ...
            "Lighting & Motor Calculation",
        ],
        "pro_title": "ElecCalc PRO",
    ...
    "tr": {
        ...
        "hero_2": "Elektrik Hesaplayıcı",
        "hero_sub_1": "Mühendislik hesaplamalarınız",
        "hero_sub_2": "artık cebinizde",
        "feat_title_1": "Tüm Hesaplamalar",
        "feat_title_2": "Tek Yerde",
        "features": [
            "Ohm Kanunu & Güç Hesabı",
            "Kablo Kesiti & Gerilim Düşüşü",
            "Kompanzasyon & Trafo Seçimi",
            "Kısa Devre & Topraklama",
            "Aydınlatma & Motor Hesabı",
        ],
        "pro_title": "ElecCalc PRO",
        "pro_sub": "Sınırsız erişim, 7 gün ücretsiz",
        "benefits": [
            "15 profesyonel hesaplayıcı",
            "Sürekli güncelleme ve yeni özellikler",
            "Sınırsız erişim",
        ],
        "monthly": "₺49.99 / ay",
...
def add_phone_frame(canvas, screenshot_path, pos, phone_size, corner_radius=40):
    """Frameless modern style: rounded screenshot + drop shadow, no bezel."""
...
def generate_preview_1(size, prefix, lang, screenshots):
    """Hero: Orange gradient + device mockup showing home screen."""
    w, h = size
    s = w / 1242
    is_ipad = prefix.startswith("ipad")
    t = TEXTS[lang]

    canvas = create_gradient(size, (255, 165, 30), (255, 94, 0)).convert("RGBA")
...
def generate_preview_2(size, prefix, lang, screenshots):
    ...
    draw.rectangle([(0, 0), (w, int(10*s))], fill=ORANGE)
    ...
    text_centered(draw, t["feat_title_2"], int(100*s + 85*s), tf, ORANGE, w)
...
def generate_preview_3(size, prefix, lang, screenshots):
    """PRO: Dark bg + orange glow + benefits + paywall mockup."""
    w, h = size
    s = w / 1242
    is_ipad = prefix.startswith("ipad")
    t = TEXTS[lang]
    ...
    for r in range(int(500*s), 0, -3):
        ...
        gd.ellipse([(cx-r, int(100*s)-r), (cx+r, int(100*s)+r)], fill=(255, 149, 0, a))
    ...
    text_centered(draw, t["pro_title"], int(100*s), tf, ORANGE, w)
    ...
    yx = w//2 + gap//2
    draw.rounded_rectangle([(yx, py), (yx+bw, py+bh)], radius=int(20*s), fill=ORANGE)
    ...
    # Use home screenshots for both locales to avoid stale paywall capture content.
    if is_ipad:
        pw = int(880*s)
        ph2 = int(1170*s)
        ppx = (w - pw) // 2
        ppy = int(780*s)
        shot = screenshots["home_ipad"]
